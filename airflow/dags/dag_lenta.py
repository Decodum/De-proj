...
from lenta import fetch_rss_lenta_initial, fetch_rss_lenta_incremental
...
with DAG(dag_id='give_me_news_lenta',
         default_args=default_args,
         description="give_me_news DAG",
         schedule_interval=timedelta(minutes=10),
         tags=["bash", "Julia"],
    ...
    good_morning = BashOperator(
        task_id='good_morning',
        bash_command=f"echo 'give_me_news_lenta' && mkdir -p /tmp/test")
     
    fetch_data_initial_task = PythonOperator(
        task_id="fetch_lenta_initial",
        python_callable=fetch_rss_lenta_initial
    )

    fetch_data_incremental_task = PythonOperator(
        task_id="fetch_lenta_incremental",
        python_callable=fetch_rss_lenta_incremental
    )
